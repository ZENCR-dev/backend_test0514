...
describe('OrchestrationService', () => {
  ...
  describe('handlePaymentSucceeded', () => {
    it('should update order status and broadcast event on payment success', async () => {
      ...
      // 验证WebSocket广播调用
      expect(mockOrchestrationGateway.broadcastPaymentSucceeded).toHaveBeenCalledWith(paymentEvent);
    });
    ...
    it('should handle errors and trigger compensation', async () => {
      ...
      // 调用被测试方法并捕获预期的错误
      await expect(service.handlePaymentSucceeded(paymentEvent)).rejects.toThrow();
      ...
      // 验证补偿事件广播
      expect(mockOrchestrationGateway.broadcastOrderCompensation).toHaveBeenCalled();
    });
    ...
    it('should validate input and throw error for invalid payload', async () => {
      ...
      // 调用被测试方法并期望抛出错误
      await expect(service.handlePaymentSucceeded(invalidPaymentEvent)).rejects.toThrow('Invalid payment success event');
    });
  });
  ...
  describe('handlePaymentFailed', () => {
    it('should update order status and broadcast event on payment failure', async () => {
      ...
      // 验证WebSocket广播调用
      expect(mockOrchestrationGateway.broadcastPaymentFailed).toHaveBeenCalledWith(paymentEvent);
    });
  ...
  describe('handleOrderStatusChanged', () => {
    it('should broadcast order status change event', async () => {
      ...
      // 验证WebSocket广播调用
      expect(mockOrchestrationGateway.broadcastOrderStatusChanged).toHaveBeenCalledWith(orderEvent);
    });
    ...
    it('should handle validation errors for invalid event data', async () => {
      ...
      // 验证没有广播状态变更
      expect(mockOrchestrationGateway.broadcastOrderStatusChanged).not.toHaveBeenCalled();
    });
  });
