...
describe('OrchestrationGateway', () => {
  ...
  describe('handleConnection', () => {
    ...
    it('should disconnect previous connection when same user connects again', async () => {
      // 模拟JWT验证和用户验证
      const mockUser = { id: 'user-123', email: 'user@example.com', role: 'doctor' };
      mockJwtService.verify.mockReturnValue({ sub: 'user-123' });
      mockAuthService.verifyPayload.mockResolvedValue(mockUser);
      
      // 创建第一个连接
      const firstSocket = { ...mockSocket, id: 'socket-1' } as unknown as Socket;
      await gateway.handleConnection(firstSocket);
      
      // 创建第二个连接（同一用户）
      const secondSocket = { ...mockSocket, id: 'socket-2' } as unknown as Socket;
  ...
  describe('broadcasting and messaging', () => {
    ...
    it('should return false when sending to non-connected user', () => {
      // 发送事件到不存在的用户
      const eventData = { message: 'test message' };
      const result = gateway.sendToUser('non-existent-user', 'test-event', eventData);
    ...
    it('should send events to users in specific clinic', async () => {
      // 首先连接两个属于同一诊所的客户端
      const mockUser1 = { id: 'user-1', email: 'user1@example.com', role: 'doctor', clinicId: 'clinic-123' };
      const mockUser2 = { id: 'user-2', email: 'user2@example.com', role: 'nurse', clinicId: 'clinic-123' };
      const mockUser3 = { id: 'user-3', email: 'user3@example.com', role: 'doctor', clinicId: 'clinic-456' };
      ...
      const sentCount = gateway.sendToClinic('clinic-123', 'clinic-event', eventData);
  ...
  describe('health and metrics', () => {
    ...
    it('should return detailed metrics', async () => {
      // 连接一个用户
      const mockUser = { id: 'user-123', email: 'user@example.com', role: 'doctor', clinicId: 'clinic-123' };
      mockJwtService.verify.mockReturnValue({});
      mockAuthService.verifyPayload.mockResolvedValue(mockUser);
      ...
      // 验证详细指标格式
      expect(detailedMetrics).toEqual(expect.objectContaining({
        activeConnections: expect.any(Number),
        totalConnectionAttempts: expect.any(Number),
        totalEvents: expect.any(Number),
        errorRate: expect.any(Number),
