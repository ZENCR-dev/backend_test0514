...
export class OrchestrationHealthController {
      ...
      example: {
        ...
        services: {
          ...
          websocket: {
            status: "down",
            connections: 0,
            details: { reason: "Socket server not running" },
          },
        },
      },
    },
  ...
  healthCheck(): HealthCheckResponse {
    ...
    const overallStatus =
      orchestrationHealthy && websocketHealthy ? "healthy" : "unhealthy";
  ...
  getMetrics(): MetricsResponse {
    ...
    const gatewayDetailedMetrics =
      this.orchestrationGateway.getDetailedMetrics();
  ...
  getDashboard(@Query("timeRange") timeRange?: string): DashboardResponse {
    this.logger.log(
      `Dashboard data requested with time range: ${timeRange || "default"}`,
    );
    ...
    // 计算总体摘要
    const overallHealth =
      serviceMetrics.healthMetrics.isHealthy &&
      this.orchestrationGateway.isHealthy()
        ? "healthy"
        : "unhealthy";
    ...
    const response: DashboardResponse = {
      ...
      summary: {
        ...
        totalEventsProcessed:
          serviceMetrics.eventProcessing.totalEventsProcessed,
        averageResponseTime:
          serviceMetrics.eventProcessing.averageProcessingTime,
  ...
  getPerformanceAnalysis(@Query("metric") metric?: string) {
    this.logger.log(
      `Performance analysis requested for metric: ${metric || "all"}`,
    );
  ...
  getRealtimeMetrics() {
    ...
    const response = {
      ...
      realtime: {
        ...
        last1MinuteMessages:
          gatewayMetrics.slidingWindowMetrics.last1MinuteMessages,
        last1MinuteErrors:
          gatewayMetrics.slidingWindowMetrics.last1MinuteErrors,
  ...
  async getEvents(
    ...
    this.logger.log(
      `Events query requested with filters: ${JSON.stringify({ eventType, processingStatus, startDate, endDate, page, limit })}`,
    );
      ...
      switch (event.eventType) {
        ...
        default:
          throw new BadRequestException(
            `Event type ${event.eventType} cannot be replayed`,
          );
      }
