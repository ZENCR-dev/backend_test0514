...
export class OrchestrationService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.error(`Failed to initialize OrchestrationService: ${error.message}`, error.stack);
      
      // 重试初始化
      if (this.retryCount < this.maxRetries) {
        this.retryCount++;
        this.logger.log(`Retrying initialization (${this.retryCount}/${this.maxRetries}) in ${this.retryDelay}ms...`);
        setTimeout(() => this.onModuleInit(), this.retryDelay);
      } else {
        this.logger.error(`Failed to initialize OrchestrationService after ${this.maxRetries} attempts`);
        throw error;
      }
    }
  }
  ...
  private setupEventListeners() {
    ...
    this.logger.log(`Listening for payment events: ${PAYMENT_EVENTS.PAYMENT_SUCCEEDED}, ${PAYMENT_EVENTS.PAYMENT_FAILED}`);
    this.logger.log(`Listening for order events: ${ORDER_EVENTS.ORDER_STATUS_CHANGED}`);
  }
  ...
  async handlePaymentSucceeded(payload: PaymentSucceededEvent) {
    ...
    try {
      ...
      this.logger.error(`Failed to persist payment succeeded event: ${error.message}`, error.stack);
      // 持久化失败不应阻止业务流程，继续处理
    }
    
    try {
      this.logger.log(`Payment succeeded for order ${payload.orderId}: ${JSON.stringify(payload)}`);
      ...
      // 调用订单服务更新状态
      const updatedOrder = await this.orderService.updateOrderStatus(payload.orderId, updateData)
        .catch(error => {
          this.logger.error(`Failed to update order status for ${payload.orderId}: ${error.message}`, error.stack);
          
          // 发布错误事件
          this.emitErrorEvent('ORDER_UPDATE_FAILED', `Failed to update order status: ${error.message}`, {
            orderId: payload.orderId,
            paymentId: payload.paymentId,
          });
      ...
      this.logger.error(`Error handling payment success for order ${payload.orderId}: ${error.message}`, error.stack);
      ...
      if (eventLog) {
        try {
          ...
          this.logger.error(`Failed to update event status: ${updateError.message}`, updateError.stack);
        }
      }
      ...
      // 尝试触发补偿机制
      this.triggerCompensation(payload.orderId, 'PAYMENT_SUCCESS_HANDLING_FAILED', error.message);
  ...
  async handlePaymentFailed(payload: PaymentFailedEvent) {
    ...
    try {
      ...
      this.logger.error(`Failed to persist payment failed event: ${error.message}`, error.stack);
      // 持久化失败不应阻止业务流程，继续处理
    }
    
    try {
      this.logger.log(`Payment failed for order ${payload.orderId}: ${JSON.stringify(payload)}`);
      ...
      this.logger.log(`Order ${payload.orderId} status updated to PAYMENT_FAILED`);
      ...
      this.logger.error(`Error handling payment failure for order ${payload.orderId}: ${error.message}`, error.stack);
  ...
  async handleOrderStatusChanged(payload: OrderStatusChangedEvent) {
    ...
    try {
      ...
      this.logger.error(`Failed to persist order status changed event: ${error.message}`, error.stack);
      // 持久化失败不应阻止业务流程，继续处理
    }
    ...
    try {
      ...
      if (!payload.orderId || !payload.newStatus) {
        throw new Error('Invalid order status change event: missing required fields');
      }
      ...
      this.logger.error(`Error handling order status change for order ${payload.orderId}: ${error.message}`, error.stack);
      ...
      // 发布错误事件
      this.emitErrorEvent('ORDER_STATUS_HANDLING_FAILED', `Failed to process order status change: ${error.message}`, {
        orderId: payload.orderId,
    ...
    try {
      switch (newStatus) {
        case OrderStatus.PROCESSING:
          // 订单开始处理，可能需要通知药房或医生
          if (payload.clinicId) {
            this.orchestrationGateway.sendToClinic(payload.clinicId, ORCHESTRATION_EVENTS.ORDER_PROCESSING, {
              orderId,
              timestamp: new Date().toISOString(),
        ...
        case OrderStatus.READY_FOR_PICKUP:
          // 订单准备好取货，通知患者
          if (payload.userId) {
            this.orchestrationGateway.sendToUser(payload.userId, ORCHESTRATION_EVENTS.ORDER_READY, {
              orderId,
              timestamp: new Date().toISOString(),
        ...
        case OrderStatus.CANCELLED:
          // 订单取消，可能需要触发退款流程
          this.logger.log(`Order ${orderId} was cancelled, may need refund processing`);
      ...
      this.logger.error(`Error in processOrderStatusChange for order ${orderId}: ${error.message}`, error.stack);
      throw error;
    }
  }
   ...
   */
  private async triggerCompensation(orderId: string, reason: string, errorMessage: string) {
    try {
      this.logger.log(`Triggering compensation for order ${orderId}: ${reason}`);
      ...
      try {
        ...
        this.logger.error(`Failed to persist compensation event: ${error.message}`, error.stack);
      }
      
      // 发布补偿事件
      this.eventEmitter.emit(ORCHESTRATION_EVENTS.ORDER_COMPENSATION, compensationEvent);
      
      // 广播补偿事件到客户端
      this.orchestrationGateway.broadcastOrderCompensation(compensationEvent);
      
    } catch (error) {
      this.logger.error(`Failed to trigger compensation for order ${orderId}: ${error.message}`, error.stack);
    }
  }
   ...
   */
  private async emitErrorEvent(code: string, message: string, metadata: Record<string, any> = {}) {
    try {
      // 记录错误
      this.errorCount++;
      ...
      try {
        ...
        this.logger.error(`Failed to persist error event: ${error.message}`, error.stack);
      }
      ...
      this.logger.error(`Failed to emit error event: ${error.message}`, error.stack);
    }
  }
  ...
  getEnhancedMetrics(): ServiceMetrics {
    ...
    // 计算处理时间统计
    const sortedProcessingTimes = [...this.eventProcessingTimes].sort((a, b) => a - b);
    const averageProcessingTime = this.eventProcessingTimes.length > 0 
      ? this.eventProcessingTimes.reduce((sum, time) => sum + time, 0) / this.eventProcessingTimes.length 
      : 0;
    ...
    return {
      eventProcessing: {
        ...
        successRate: totalEvents > 0 ? (this.eventSuccessCount / totalEvents) * 100 : 0,
        failureRate: totalEvents > 0 ? (this.eventFailureCount / totalEvents) * 100 : 0,
      },
  ...
  async retryWithExponentialBackoff<T>(
    ...
    try {
      ...
      this.logger.warn(`Retrying ${context} (attempt ${retryCount + 1}/${this.enhancedRetryConfig.maxRetries}) after ${delay}ms. Error: ${error.message}`);
  ...
  private addToDeadLetterQueue(item: any): void {
    ...
    this.logger.error(`Added item to dead letter queue: ${JSON.stringify(item)}`);
  }
  ...
  private checkAndTriggerAlerts(): void {
    ...
    if (errorRate > this.alertConfig.errorRateThreshold) {
      this.triggerAlert('HIGH_ERROR_RATE', `Error rate ${errorRate}% exceeds threshold ${this.alertConfig.errorRateThreshold}%`);
    }

    // 死信队列大小告警
    if (this.deadLetterQueue.length > this.alertConfig.deadLetterQueueSizeThreshold) {
      this.triggerAlert('DEAD_LETTER_QUEUE_FULL', `Dead letter queue size ${this.deadLetterQueue.length} exceeds threshold ${this.alertConfig.deadLetterQueueSizeThreshold}`);
    }

    // 连续失败告警
    if (this.consecutiveFailures > this.alertConfig.consecutiveFailuresThreshold) {
      this.triggerAlert('CONSECUTIVE_FAILURES', `${this.consecutiveFailures} consecutive failures exceed threshold ${this.alertConfig.consecutiveFailuresThreshold}`);
    }
  }
   ...
   * 重新处理死信队列中的事件
   */
  async reprocessDeadLetterQueue(): Promise<{ processed: number; failed: number }> {
