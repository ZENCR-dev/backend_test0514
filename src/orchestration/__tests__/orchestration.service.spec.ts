...
import { PaymentSucceededEvent, PaymentFailedEvent } from '../../common/events/types';
...
describe('OrchestrationService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OrchestrationService,
        {
          ...
          useValue: {
            updateOrderStatus: jest.fn().mockImplementation((orderId, updateData) => {
              // 模拟成功更新订单状态
  ...
  describe('handlePaymentSucceeded', () => {
    it('should update order status to PAID when payment succeeds', async () => {
      ...
      // 验证WebSocket事件广播
      expect(gateway.broadcastPaymentSucceeded).toHaveBeenCalledWith(mockPaymentSucceededEvent);
    });

    it('should handle errors during payment success processing', async () => {
      // 模拟订单服务抛出异常
      (orderService.updateOrderStatus as jest.Mock).mockRejectedValueOnce(new Error('Order update failed'));

      // 调用方法并期待抛出异常
      await expect(service.handlePaymentSucceeded(mockPaymentSucceededEvent)).rejects.toThrow('Order update failed');
  ...
  describe('handlePaymentFailed', () => {
    it('should update order status to PAYMENT_FAILED when payment fails', async () => {
      ...
      // 验证WebSocket事件广播
      expect(gateway.broadcastPaymentFailed).toHaveBeenCalledWith(mockPaymentFailedEvent);
    });
    ...
    it('should handle errors during payment failure processing', async () => {
      ...
      // 调用方法并期待抛出异常
      await expect(service.handlePaymentFailed(mockPaymentFailedEvent)).rejects.toThrow('Order update failed');
