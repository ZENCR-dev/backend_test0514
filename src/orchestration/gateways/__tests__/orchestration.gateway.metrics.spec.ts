...
describe("OrchestrationGateway - Enhanced Metrics", () => {
  ...
  describe("Enhanced Metrics", () => {
    ...
    it("should track room/channel statistics", async () => {
      ...
      // 设置authService返回不同用户
      authService.verifyPayload = jest
        .fn()
    ...
    it("should implement sliding window metrics", async () => {
      ...
      expect(
        metrics.slidingWindowMetrics.last1MinuteMessages,
      ).toBeGreaterThanOrEqual(5);
      expect(
        metrics.slidingWindowMetrics.last5MinuteMessages,
      ).toBeGreaterThanOrEqual(5);
    });
  ...
  describe("Performance Metrics", () => {
    it("should calculate percentile latencies", async () => {
      ...
      expect(metrics.performanceMetrics.p95Latency).toBeGreaterThanOrEqual(
        metrics.performanceMetrics.p50Latency,
      );
    });
    ...
    it("should reset peak connections", async () => {
      ...
      expect(metrics.connectionMetrics.peakConnections).toBe(
        currentConnections,
      ); // 重置为当前连接数
    });
  });
