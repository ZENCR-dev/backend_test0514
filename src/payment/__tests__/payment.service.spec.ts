...
describe('PaymentService - STRIPE-01 Payment Intent基础功能', () => {
  ...
  const mockStripePaymentIntent = {
    ...
    charges: { object: 'list', data: [], has_more: false, total_count: 0, url: '/v1/charges' },
  ...
  describe('createPaymentIntent', () => {
    beforeEach(() => {
      // 默认Mock行为：无重复支付
      jest.spyOn(service as any, 'checkDuplicatePayment').mockResolvedValue(false);
      mockStripe.paymentIntents.create.mockResolvedValue(mockStripePaymentIntent);
    });

    describe('成功场景', () => {
      it('应该成功创建支付意图并返回正确的响应格式', async () => {
        const result = await service.createPaymentIntent(mockCreatePaymentIntentRequest);
    ...
    describe('验证和错误处理', () => {
      it('应该拒绝金额过小的支付请求', async () => {
        ...
        await expect(service.createPaymentIntent(requestWithSmallAmount))
      ...
      it('应该拒绝金额过大的支付请求', async () => {
        ...
        await expect(service.createPaymentIntent(requestWithLargeAmount))
          .rejects.toThrow(PaymentIntentCreationException);
      });
      ...
      it('应该检测并拒绝重复支付', async () => {
        ...
        await expect(service.createPaymentIntent(mockCreatePaymentIntentRequest))
    ...
    describe('幂等性和重复检测', () => {
      it('应该调用重复支付检测', async () => {
        const checkDuplicateSpy = jest.spyOn(service as any, 'checkDuplicatePayment');
  ...
  describe('getPaymentIntent', () => {
    ...
    describe('成功场景', () => {
      ...
      it('应该正确映射不同的Stripe状态', async () => {
        const testCases = [
          { stripeStatus: 'requires_payment_method', expectedStatus: PaymentStatus.REQUIRES_PAYMENT_METHOD },
          { stripeStatus: 'requires_confirmation', expectedStatus: PaymentStatus.REQUIRES_CONFIRMATION },
          { stripeStatus: 'requires_action', expectedStatus: PaymentStatus.REQUIRES_ACTION },
          { stripeStatus: 'processing', expectedStatus: PaymentStatus.PROCESSING },
          { stripeStatus: 'succeeded', expectedStatus: PaymentStatus.SUCCEEDED },
    ...
    describe('错误处理', () => {
      ...
      it('应该验证支付意图ID格式', async () => {
        await expect(service.getPaymentIntent(''))
          .rejects.toThrow('Payment intent ID is required');

        await expect(service.getPaymentIntent('invalid-id'))
  ...
  describe('cancelPaymentIntent', () => {
    ...
    describe('成功场景', () => {
      it('应该成功取消支付意图', async () => {
        await expect(service.cancelPaymentIntent(paymentIntentId))
      ...
      it('应该记录取消操作的日志', async () => {
        ...
        expect(loggerSpy).toHaveBeenCalledWith(
          expect.stringContaining(`Cancelling payment intent: ${paymentIntentId}`)
        );
        expect(loggerSpy).toHaveBeenCalledWith(
          expect.stringContaining(`Payment intent cancelled successfully: ${paymentIntentId}`)
        );
      });
    });
    ...
    describe('错误处理', () => {
      ...
      it('应该处理已经取消的支付意图', async () => {
        ...
        await expect(service.cancelPaymentIntent(paymentIntentId))
          .rejects.toThrow('Payment intent is already canceled or cannot be canceled');
      });
      ...
      it('应该处理已经成功的支付意图', async () => {
        ...
        await expect(service.cancelPaymentIntent(paymentIntentId))
          .rejects.toThrow('Payment intent is already succeeded and cannot be canceled');
      });

      it('应该验证支付意图ID格式', async () => {
        await expect(service.cancelPaymentIntent(''))
          .rejects.toThrow('Payment intent ID is required');

        await expect(service.cancelPaymentIntent('invalid-id'))
          .rejects.toThrow('Invalid payment intent ID format');
      });
  ...
  describe('辅助方法测试', () => {
    describe('mapStripeStatusToPaymentStatus', () => {
      it('应该正确映射所有Stripe状态', () => {
        const mapMethod = (service as any).mapStripeStatusToPaymentStatus.bind(service);

        expect(mapMethod('requires_payment_method')).toBe(PaymentStatus.REQUIRES_PAYMENT_METHOD);
        expect(mapMethod('requires_confirmation')).toBe(PaymentStatus.REQUIRES_CONFIRMATION);
        expect(mapMethod('requires_action')).toBe(PaymentStatus.REQUIRES_ACTION);
    ...
    describe('validatePaymentIntentId', () => {
      it('应该验证支付意图ID格式', () => {
        const validateMethod = (service as any).validatePaymentIntentId?.bind(service);
        
        if (validateMethod) {
          expect(() => validateMethod('')).toThrow('Payment intent ID is required');
          expect(() => validateMethod('invalid-id')).toThrow('Invalid payment intent ID format');
