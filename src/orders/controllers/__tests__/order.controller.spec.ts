...
import { OrderResponseDto, ApiResponseWrapper, PaginatedOrderResponseDto } from '../../dto/order-response.dto';
import { OrderStatus } from '@prisma/client';
import { IOrder, IOrderItem, IPaginatedOrders } from '../../interfaces/order-management.interface';
...
describe('OrderController', () => {
  ...
  const createMockUser = (role: string, id: string = 'user-123', clinicId: string = 'clinic-123') => ({
    id,
    role,
    clinicId,
    email: `${role}@test.com`
  ...
  const createMockOrderItem = (overrides: Partial<IOrderItem> = {}): IOrderItem => ({
    id: 'item-123',
    medicineId: 'med-123',
  ...
  const createMockPaginatedResult = (orders: IOrder[] = [createMockOrder()]): IPaginatedOrders => ({
    data: orders,
    total: orders.length,
    page: 1,
    limit: 10,
  ...
  beforeEach(async () => {
    ...
    mockOrderService = module.get<OrderService>(OrderService) as jest.Mocked<OrderService>;
  });
  ...
  describe('findAll (GET /orders)', () => {
    it('should return paginated orders for a doctor', async () => {
      ...
      expect(mockOrderService.queryOrders).toHaveBeenCalledWith({ ...query, practitionerId: user.id });
    });

    it('should allow admin to query orders without practitionerId restriction', async () => {
        const user = createMockUser('admin');
        const req = createMockRequest(user);
        const query: QueryOrderDto = { page: 1, limit: 10, practitionerId: 'any-doctor' };
        const paginatedResult = createMockPaginatedResult();
        mockOrderService.queryOrders.mockResolvedValue(paginatedResult);
  ...
  describe('findOne (GET /orders/:id)', () => {
    ...
    it('should throw ForbiddenException for unauthorized access', async () => {
      ...
      await expect(controller.findOne('order-123', req)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should throw NotFoundException if order does not exist', async () => {
        ...
        await expect(controller.findOne('not-found-id', req)).rejects.toThrow(NotFoundException);
      });
  });
  ...
  describe('update (PATCH /orders/:id)', () => {
    it('should update an order successfully', async () => {
      ...
      const existingOrder = createMockOrder({ practitionerId: 'doctor-123', version: 1 });
      ...
      expect(mockOrderService.updateOrder).toHaveBeenCalledWith('order-123', updateDto);
    });
    ...
    it('should throw ConflictException on version mismatch', async () => {
        ...
        const req = createMockRequest(user);
        const updateDto: UpdateOrderDto = { notes: 'updated note', version: 1 };
        const existingOrder = createMockOrder({ practitionerId: 'doctor-123', version: 2 });
  
        mockOrderService.getOrderById.mockResolvedValue(existingOrder);
  
        await expect(controller.update('order-123', updateDto, req)).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('remove (DELETE /orders/:id)', () => {
    it('should cancel an order successfully', async () => {
      ...
      const existingOrder = createMockOrder({ practitionerId: 'doctor-123', status: OrderStatus.DRAFT });
    ...
    it('should throw BadRequestException if order is not in cancellable state', async () => {
        ...
        const existingOrder = createMockOrder({ practitionerId: 'doctor-123', status: OrderStatus.PAID });
  
        mockOrderService.getOrderById.mockResolvedValue(existingOrder);
  
        await expect(controller.remove('order-123', req)).rejects.toThrow(BadRequestException);
      });
  });
}); 