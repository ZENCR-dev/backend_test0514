...
import { IOrder, IPaginatedOrders, IOrderQueryCriteria } from '../interfaces/order-management.interface';
...
export class OrderController {
  ...
  @ApiOperation({ 
    summary: '分页查询订单列表',
    description: '根据查询条件分页获取订单列表，支持按状态、诊所等条件过滤。医生只能查看自己的订单，管理员可以查看所有订单。'
  })
  @ApiResponse({ 
    status: 200, 
    description: '查询成功',
  ...
  async findOne(@Param('id') id: string, @Req() req: any): Promise<ApiResponseWrapper<OrderResponseDto>> {
   ...
   */
  @Post()
  @RequirePermissions({ action: Action.CREATE, resource: Resource.PRESCRIPTION })
  @ApiOperation({ 
    summary: '创建新订单',
   ...
   */
  @Patch(':id')
  @RequirePermissions({ action: Action.UPDATE, resource: Resource.PRESCRIPTION })
  @ApiOperation({ 
    summary: '更新订单信息',
   ...
   */
  @Delete(':id')
  @RequirePermissions({ action: Action.DELETE, resource: Resource.PRESCRIPTION })
  @ApiOperation({ 
    summary: '取消订单',
  ...
  @ApiBody({ 
    ...
    schema: { type: 'object', properties: { version: { type: 'number', example: 1 } } }
  })
  @ApiResponse({ status: 200, description: '订单取消成功', type: ApiResponseWrapper })
  ...
  async remove(@Param('id') id: string, @Req() req: any): Promise<ApiResponseWrapper<OrderResponseDto>> {
