...
describe('PrescriptionsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw error when medicine validation fails', async () => {
      ...
      await expect(service.create(mockPrescriptionData, 'doctor-123')).rejects.toThrow(
        '创建处方失败: 无效的药品ID'
      );
    });
    ...
    it('should handle duplicate medicine error', async () => {
      ...
      mockRepository.create.mockRejectedValue(new Error('药品列表中存在重复项'));
  ...
  describe('findAll', () => {
    it('should return paginated prescriptions for doctor', async () => {
      ...
      const result = await service.findAll('doctor-123', { page: 1, limit: 20 });

      expect(mockRepository.findByDoctor).toHaveBeenCalledWith('doctor-123', 1, 20);
      expect(result.success).toBe(true);
  ...
  describe('update', () => {
    it('should update prescription successfully', async () => {
      ...
      const result = await service.update('prescription-123', updateData, 'doctor-123');
  ...
  describe('issuePrescription', () => {
    it('should issue prescription successfully', async () => {
      ...
      const result = await service.issuePrescription('prescription-123', 'doctor-123');

      expect(mockRepository.updateStatus).toHaveBeenCalledWith('prescription-123', 'PAID');
      expect(result.success).toBe(true);
    });
  ...
  describe('verifyPrescription', () => {
    it('should verify prescription successfully', async () => {
      ...
      expect(mockQRCodeService.parseQRCodeString).toHaveBeenCalledWith('valid-qr-code');
      expect(result.success).toBe(true);
