...
describe("API Endpoints Contract Tests", () => {
  ...
  describe("认证端点连通性测试", () => {
    test(
      "POST /api/v1/auth/login - 登录端点连通性",
      async () => {
    ...
    test(
      "POST /api/v1/auth/refresh - Token刷新端点连通性",
      async () => {
  ...
  describe("药品端点连通性测试", () => {
    test(
      "GET /api/v1/medicines - 药品列表端点连通性",
      async () => {
    ...
    test(
      "GET /api/v1/medicines?search= - 药品搜索端点连通性",
      async () => {
    ...
    test(
      "GET /api/v1/medicines?page=1&limit=10 - 药品分页端点连通性",
      async () => {
        const requestAgent = request(API_BASE_URL)
          .get("/api/v1/medicines")
  ...
  describe("错误处理连通性测试", () => {
    test(
      "GET /api/v1/nonexistent - 404 错误处理",
      async () => {
        const response = await request(API_BASE_URL)
  ...
  describe("CORS 和基础配置测试", () => {
    test(
      "OPTIONS /api/v1/medicines - CORS 预检请求",
      async () => {
        const response = await request(API_BASE_URL)
